...
public class SnakeApp {
	...
	private static final Cell[] spawn = { new Cell(1, (Grid_Size.GRID_HEIGHT / 2) / 2),
			new Cell(Grid_Size.GRID_WIDTH - 2, 3 * (Grid_Size.GRID_HEIGHT / 2) / 2),
			...
			new Cell(3 * (Grid_Size.GRID_WIDTH / 2) / 2, Grid_Size.GRID_HEIGHT - 2) };
	...
	public SnakeApp() {
		...
		board.addMouseListener(new MouseListener() {
			...
			public void mousePressed(MouseEvent arg0) {
				...
				Cell c = new Cell(arg0.getX() / Grid_Size.WIDTH_BOX, arg0.getY() / Grid_Size.HEIGH_BOX);
					...
					for (int i = 0; i != MAX_THREADS; i++) {
						for (int j = 0; j != snakes[i].getBody().size(); j++) {
							if (snakes[i].getBody().get(j).getX() == c.getX()
									&& snakes[i].getBody().get(j).getY() == c.getY()) {
								snakes[i].setSelected(true);
								System.out.println("[" + snakes[i].getIdt() + "] was selected!");
